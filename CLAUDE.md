...
// 畝定義（各圃場6畝）。物理的な畝の位置とidの番号は逆（F1R6/F2R6が物理1番畝）。
// RIDGES配列の並び順＝縦一覧（入力/履歴/ガント等）の表示順。1番畝を先頭にするため
// 配列をR6→R1の順に並べ、noを1〜6で割り当てている（idそのものは変更しない）。
// 東西に横並びのマップ（圃場マップ・作付け計画の月別マップ等）は、1番畝が物理的に
// 東側にあるため、配列順をそのまま使うと西/東が逆になる。そのため横並びマップ専用の
// ridgesWestToEast(fieldId) で並びを反転して使う（西→東の描画順に変換）
...
- `ridgesWestToEast(fieldId)` — 横並びマップ用に畝を西→東の物理順で取得（RIDGES配列順とは逆）
